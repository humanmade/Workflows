...
add_action( 'plugins_loaded', function () {
	...
	// Built in workflows.
	Workflow::register( 'draft_to_pending', __( 'Notify editors by email when a post is ready to publish', 'hm-workflow' ) )
		->when( 'draft_to_pending' )
		->what( '%title% is ready to be published', [
			[
				'id'              => 'view',
				'text'            => __( 'View', 'hm-workflow' ),
				'callback_or_url' => function ( $post ) {
					return $post->ID;
				},
				'args'            => [],
				'schema'          => [],
	...
	Workflow::register( 'publish_post', __( 'Notify editors by email when a post has been published', 'hm-workflow' ) )
		->when( 'publish_post' )
		->what( '%title% has been published' )// @todo: consider i18n
		->who( 'editor' )
		->where( 'email' );

	Workflow::register( 'publish_page', __( 'Notify editors by email when a page has been published', 'hm-workflow' ) )
		->when( 'publish_page' )
		->what( '%title% has been published' )// @todo: consider i18n
		->who( 'editor' )
		->where( 'email' );
